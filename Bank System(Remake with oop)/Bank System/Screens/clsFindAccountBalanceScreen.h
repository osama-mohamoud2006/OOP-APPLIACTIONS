...
public:
	static void FindClientByAccountNumberToGetHisBalance() {
		...
		while (true)
		{
			...
			system("cls");
			cout << "\n\n\n\n";
			clsScreen::_PrintMenuOption(colorText("          Get Balance Client Screen", "yellow"));


			if (Trials > 4)
			{
				std::cout << colorText("\n\t\t\t\t\t\aYou Tried For " + to_string(Trials - 1) + " Times To Find The Account !\n\t\t\t\t\tExit To Transactions Menu[y] Or Try Again[n] ? : ", "red");
				if (clsInputAndValidation::Confirm("")) { // if user entered 'Y' then it will returns true 'confirm()'
					break; // Back to Transactions menu 
				}
				else {
					Trials = 0;
